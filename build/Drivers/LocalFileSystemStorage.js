...
class LocalFileSystemStorage extends Storage_1.default {
    ...
    async append(location, content) {
        try {
            const result = await fs_extra_1.default.appendFile(this._fullPath(location), content);
            return { raw: result };
        }
        catch (e) {
    ...
    async copy(src, dest) {
        try {
            const result = await fs_extra_1.default.copy(this._fullPath(src), this._fullPath(dest));
            return { raw: result };
        }
        catch (e) {
    ...
    async delete(location) {
        try {
            const result = await fs_extra_1.default.unlink(this._fullPath(location));
    ...
    async exists(location) {
        try {
            const result = await fs_extra_1.default.pathExists(this._fullPath(location));
    ...
    async get(location, encoding = 'utf-8') {
        try {
            const result = await fs_extra_1.default.readFile(this._fullPath(location), encoding);
    ...
    async getBuffer(location) {
        try {
            const result = await fs_extra_1.default.readFile(this._fullPath(location));
            return { content: result, raw: result };
        }
        catch (e) {
            throw handleError(e, location);
    ...
    async getStat(location) {
        try {
            const stat = await fs_extra_1.default.stat(this._fullPath(location));
            return {
                size: stat.size,
                modified: stat.mtime,
                raw: stat,
     ...
     */
    getStream(location) {
        return fs_extra_1.default.createReadStream(this._fullPath(location));
    }
    /**
    ...
    async move(src, dest) {
        try {
            const result = await fs_extra_1.default.move(this._fullPath(src), this._fullPath(dest));
            return { raw: result };
        }
        catch (e) {
            throw handleError(e, `${src} -> ${dest}`);
    ...
    async put(location, content) {
        ...
        try {
            if (utils_1.isReadableStream(content)) {
                ...
                const ws = fs_extra_1.default.createWriteStream(fullPath);
                await utils_1.pipeline(content, ws);
                return { raw: undefined };
            }
            const result = await fs_extra_1.default.outputFile(fullPath, content);
            return { raw: result };
        }
        catch (e) {
            throw handleError(e, location);
